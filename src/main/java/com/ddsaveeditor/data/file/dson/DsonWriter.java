...
import com.ddsaveeditor.data.io.IDataWriter;
...
class DsonWriter implements IDataWriter {
    private final DSONReader.HeaderBlock header;
    private final ByteArrayOutputStream data;
    private final ArrayList<DSONReader.Meta1Block.Meta1BlockEntry> meta1Entries;
    private final Deque<Integer> parentIdxStack;
    private final Deque<String> nameStack;
    private final ArrayList<DSONReader.Meta2Block.Meta2BlockEntry> meta2Entries;

    DsonWriter(String jsonData) throws IOException, ParseException, InterruptedException {
        this(new JsonFactory().createParser(jsonData));
    }

    DsonWriter(byte[] data) throws IOException, ParseException, InterruptedException {
    ...
    private DsonWriter(JsonParser reader) throws IOException, ParseException, InterruptedException {
        header = new DSONReader.HeaderBlock();
    ...
    private void writeField(String name, JsonParser reader) throws IOException, ParseException, InterruptedException {
        ...
        DSONReader.Meta2Block.Meta2BlockEntry e2 = new DSONReader.Meta2Block.Meta2BlockEntry();
        ...
        try {
            ...
            if (reader.getCurrentToken() == JsonToken.START_OBJECT) {
                if (!name.equals("raw_data") && !name.equals("static_save")) {
                    DSONReader.Meta1Block.Meta1BlockEntry e1 = new DSONReader.Meta1Block.Meta1BlockEntry();
                ...
                // field name
                nameStack.push(name);
                if (DsonTypes.isA(DsonTypes.FieldType.TYPE_FLOATARRAY, nameStack::iterator)) {
                    align();
                ...
                } else if (DsonTypes.isA(DsonTypes.FieldType.TYPE_INTVECTOR, nameStack::iterator)) {
                    align();
                    if (reader.getCurrentToken() != JsonToken.START_ARRAY) {
                        throw new ParseException("Expected [", (int) reader.getCurrentLocation().getCharOffset());
                    }
                ...
                } else if (DsonTypes.isA(DsonTypes.FieldType.TYPE_STRINGVECTOR, nameStack::iterator)) {
                    align();
                    if (reader.getCurrentToken() != JsonToken.START_ARRAY) {
                        throw new ParseException("Expected [", (int) reader.getCurrentLocation().getCharOffset());
                    }
                    ...
                    data.write(intBytes(numElem));
                    data.write(vecData.toByteArray());
                } else if (DsonTypes.isA(DsonTypes.FieldType.TYPE_FLOAT, nameStack::iterator)) {
                    align();
                    ...
                    data.write(floatBytes(reader.getFloatValue()));
                } else if (DsonTypes.isA(DsonTypes.FieldType.TYPE_TWOINT, nameStack::iterator)) {
                    align();
                    if (reader.getCurrentToken() != JsonToken.START_ARRAY) {
                        throw new ParseException("Expected [", (int) reader.getCurrentLocation().getCharOffset());
                    }
                ...
                } else if (DsonTypes.isA(DsonTypes.FieldType.TYPE_CHAR, nameStack::iterator)) {
                    if (reader.getCurrentToken() != JsonToken.VALUE_STRING) {
    ...
    public byte[] bytes() {
        ...
        for (int i = 0; i < meta1Entries.size(); i++) {
            DSONReader.Meta1Block.Meta1BlockEntry e1 = meta1Entries.get(i);
        ...
        for (int i = 0; i < meta2Entries.size(); i++) {
            DSONReader.Meta2Block.Meta2BlockEntry e2 = meta2Entries.get(i);
