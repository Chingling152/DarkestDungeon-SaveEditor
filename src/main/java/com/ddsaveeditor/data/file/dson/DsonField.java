...
/**
 * Dson field for Darkest Dungeon's Json
 */
class DsonField {
    ...
    public DsonTypes.FieldType type = DsonTypes.FieldType.TYPE_UNKNOWN;
    ...
    public DSONReader embeddedFile;
    ...
    // type
    public boolean guessType(DSONReader.UnhashBehavior behavior) throws ParseException {
        if (parseHardcodedType(behavior)) {
            return true;
        } else if (rawData.length == 1) {
            if (rawData[0] >= 0x20 && rawData[0] <= 0x7E) {
                type = DsonTypes.FieldType.TYPE_CHAR;
                dataString = "\"" + Character.toString((char) rawData[0]) + "\"";
            } else {
                type = DsonTypes.FieldType.TYPE_BOOL;
            ...
            type = DsonTypes.FieldType.TYPE_TWOBOOL;
            ...
            type = DsonTypes.FieldType.TYPE_INT;
            ...
            if (behavior == DSONReader.UnhashBehavior.UNHASH || behavior == DSONReader.UnhashBehavior.POUNDUNHASH) {
                String unHashed = DsonTypes.NAME_TABLE.get(tempInt);
                if (unHashed != null) {
                    hashedValue = dataString;
                    dataString = (behavior == DSONReader.UnhashBehavior.POUNDUNHASH) ? ("\"###" + unHashed + "\"")
                            : ("\"" + unHashed + "\"");
                }
            }
        } else if (parseString()) {
            ...
            if (dataString.length() >= 6) {
                ...
                if (Arrays.equals(tempHeader, DSONReader.MAGICNR_HEADER)) {
                    type = DsonTypes.FieldType.TYPE_FILE;
                    embeddedFile = new DSONReader(unquoteData, behavior);
    ...
    private boolean parseHardcodedType(DSONReader.UnhashBehavior behavior) {
        return parseFloatArray() || parseIntVector(behavior) || parseStringVector() || parseFloat() || parseTwoInt();
    }

    private boolean parseTwoInt() {
        if (DsonTypes.isA(DsonTypes.FieldType.TYPE_TWOINT, this::nameIterator)) {
            if (alignedSize() == 8) {
                type = DsonTypes.FieldType.TYPE_TWOINT;
    ...
    private boolean parseFloat() {
        if (DsonTypes.isA(DsonTypes.FieldType.TYPE_FLOAT, this::nameIterator)) {
            if (alignedSize() == 4) {
                type = DsonTypes.FieldType.TYPE_FLOAT;
                byte[] tempArr = Arrays.copyOfRange(rawData, alignmentSkip(), alignmentSkip() + 4);
    ...
    private boolean parseStringVector() {
        if (DsonTypes.isA(DsonTypes.FieldType.TYPE_STRINGVECTOR, this::nameIterator)) {
            type = DsonTypes.FieldType.TYPE_STRINGVECTOR;
            byte[] tempArr = Arrays.copyOfRange(rawData, alignmentSkip(), alignmentSkip() + 4);
    ...
    private boolean parseIntVector(DSONReader.UnhashBehavior behavior) {
        if (DsonTypes.isA(DsonTypes.FieldType.TYPE_INTVECTOR, this::nameIterator)) {
            byte[] tempArr = Arrays.copyOfRange(rawData, alignmentSkip(), alignmentSkip() + 4);
            int arrLen = ByteBuffer.wrap(tempArr).order(ByteOrder.LITTLE_ENDIAN).getInt();
            if (alignedSize() == (arrLen + 1) * 4) {
                type = DsonTypes.FieldType.TYPE_INTVECTOR;
                ...
                for (int i = 0; i < arrLen; i++) {
                    ...
                    if ((behavior == DSONReader.UnhashBehavior.UNHASH || behavior == DSONReader.UnhashBehavior.POUNDUNHASH)
                        ...
                        unHashed = (behavior == DSONReader.UnhashBehavior.POUNDUNHASH) ? ("\"###" + unHashed + "\"")
                                : ("\"" + unHashed + "\"");
                        sb.append(unHashed);
                        hsb.append(tempInt);
                        foundHashed = true;
    ...
    private boolean parseFloatArray() {
        if (DsonTypes.isA(DsonTypes.FieldType.TYPE_FLOATARRAY, this::nameIterator)) {
            type = DsonTypes.FieldType.TYPE_FLOATARRAY;
    ...
    private boolean parseString() {
        ...
        if (alignedSize() >= 5) {
            ...
            if (alignedSize() == 4 + strlen) {
                type = DsonTypes.FieldType.TYPE_STRING;
    ...
    public String getExtraComments() {
        ...
        if (type == DsonTypes.FieldType.TYPE_UNKNOWN) {
            sb.append(", Raw Data: ");
            sb.append(DSONReader.LEBytesToHexStr(rawData));
        }
        return sb.toString();
    }
